...
function openFacultyInfoModal(facId, name) {
    ...
    // Appended network cache-buster timestamp parameter to bypass proxy structures
    fetch(`get_faculty_info.php?id=${facId}&name=${encodeURIComponent(name)}&t=${new Date().getTime()}`)
        .then(r => r.json())
        .then(data => {
            if (data.error) { 
            ...
            if (data.profile_picture && data.profile_picture.trim() !== '') {
                // Pointing out of php/ directly into the sibling uploads/ directory
